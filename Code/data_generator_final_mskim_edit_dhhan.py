...
def replace_entity(sentences, scripts):
    # # place = ['burger king', 'burgerking', 'restroom', 'restroom facility', 'supermarket', 'school', 'starbucks',
    # #          'coffee shop', 'cafe', 'college', 'gas station', 'park', 'department store', 'restaurant',
    # #          'indoor parking', 'outdoor parking', 'destination']
    # place = ['burger king', 'restroom', 'supermarket', 'school', 'Starbucks',
    #          'coffee shop', 'cafe', 'college', 'gas station', 'park', 'department store', 'restaurant']
    ...
    # time = ['tomorrow', 'the day after tomorrow', 'next week', 'afternoon', 'morning', 'evening', 'dawn', 'midnight',
    #         'noon']
    # distance = ['50 meters', '100 meters', '500 meters']
    # special_route = ['ALTROUTE', 'ETA', 'REROUTE', 'route']
    # special_mode = ['DRIVERANGE']
    # # range_distance = ['nearby', '']
    # service = ['valet service', 'credit card', 'takeout', 'delivery']
    # search_place = ['San Francisco Museum of Modern Art', 'Downtown Berkeley', 'Bay Bridge', 'Oakland', 'Seoul',
    #                 'New York', 'Wahington', 'Chicago']
    # Keywords = ['[place]', '[other]', '[weather]', '[geocode]', '[event]', '[time]', '[distance]', '[service]',
    #             '[search_place]', '[special_route]', '[special_mode]']
    # entity = [place, other, weather, geocode, event, time, distance, service, search_place, special_route, special_mode]
    # class_lists = []
    # g_sentence = []
    # g_scripts = []
    # number=0
    ...
    #          'indoor parking', 'outdoor parking', 'destination']
    place = ['burger king', 'restroom', 'supermarket', 'school', 'Starbucks',
             'coffee shop', 'cafe', 'college', 'gas station', 'park', 'restaurant']
    other = ['route', 'traffic camera', 'traffic', 'speed camera']
    weather = ['weather', 'weather forecast']
    geocode = ['US-101', 'Bayshore Blvd', 'I-580 East']
    event = ['meeting', 'party', 'speech']
    ...
    # range_distance = ['nearby', '']
    service = ['valet service', 'credit card', 'takeout', 'delivery']
    search_place = ['San Francisco Museum of Modern Art', 'Bay Bridge', 'Seoul', 'New York', 'Wahington', 'Chicago']
    Keywords = ['[place]', '[other]', '[weather]', '[geocode]', '[event]', '[time]', '[distance]', '[service]',
                '[search_place]', '[special_route]', '[special_mode]']
    entity = [place, other, weather, geocode, event, time, distance, service, search_place, special_route, special_mode]
    class_lists = []
    ...
    for sentence, script in zip(sentences, scripts):
        ...
        for K in Keywords: ###########while로 바꾸어야함
            print(sentence)
            if K in replaced_sentence[0]:
                ...
                replaced_sentence = [temp[a].replace(sentence, entity[Keywords.index(sentence)][j],1)
                                         ...
                                         for j in range(len(entity[Keywords.index(sentence)])) if (entity[Keywords.index(sentence)][j] not in temp[a])]
            ...
            # for pre_sentence in replaced_sentence:
            #     if K in pre_sentence:
...
def make_data():
    sentences = [
            ...
            "What's my [special_route].",
            "Show me [special_route].",
            "[special_route] using [geocode].",

            "Show [other] overview.",
            "Drive to [search_place].",
            "What's my [special_mode].",
    ...
    navscripts = [
        ...
        "[MODE [other] WHERE:ROUTE WITH:[VOICERESPONSE TEMPLATE:""*]]",
    ...
    with open('Test_0827.txt', 'w') as f:
