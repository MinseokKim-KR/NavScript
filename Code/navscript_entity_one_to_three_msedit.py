...
for test_enum, Input_data in enumerate(zip(t_sentences,t_scripts,class_ID)):
    ...
    #sess.run([tf.global_variables_initializer(), tf.tables_initializer()])
    test_message_embeddings = sess.run(embed([result]))
    ...
    if str(embeded_class_id[entity_num-1][minimum_index]) != str(Input_data[2]):
        Wrong_sentence = Input_data[0]
        Wrong_script = Input_data[1]
        Wrong_classID= embeded_class_id[entity_num-1][minimum_index]
        Wrong_answer = Input_data[2]
        if result2.upper().replace(" ","") != Input_data[1].upper().replace(" ",""):
            Wrong_navscript="X"
            w.write(str(Wrong_answer) + "||" + str(Wrong_classID) + "||" + str(Wrong_script) + "||" + "X" + "||" + str(Wrong_navscript) + "\n")
        else:
            Wrong_navscript = "O"
            Correct_nav_num = Correct_nav_num+1
            w.write(str(Wrong_answer) + "||" + str(Wrong_classID) + "||" + str(Wrong_script) + "||" + "O" + "||" + str(Wrong_navscript) + "\n")
    else:
        Correct_sentence = Input_data[0]
        Correct_script = Input_data[1]
        Correct_classID = embeded_class_id[entity_num - 1][minimum_index]
        Correct_answer = Input_data[2]
        if result2.upper().replace(" ","") != Input_data[1].upper().replace(" ",""):
            Correct_navscript= "X"
            Correct_ID_num = Correct_ID_num +1
            w.write(str(Correct_answer) + "||" + str(Correct_classID) + "||" + str(Correct_script) + "||" + "X" + "||" + str(Correct_navscript) + "\n")
        else:
            Correct_navscript = "O"
            Correct_nav_num = Correct_nav_num + 1
            Correct_all = Correct_all + 1
            w.write(str(Correct_answer) + "||" + str(Correct_classID) + "||" + str(Correct_script) + "||" + "O" + "||" + str(Correct_navscript) + "\n")
...
# w.write("Accuracy : "+len(Correct_answer)/len(t_sentences)*100)

w.write("Accuracy ID : "+Correct_ID_num/len(t_sentences)*100)
w.write("Accuracy nav : "+Correct_nav_num/len(t_sentences)*100)
w.write("Accuracy all: "+Correct_all/len(t_sentences)*100)
w.close()
